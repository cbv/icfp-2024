...
/*
    The minimp3 and minimp3_ex header/implementations are from:
...
bool MP3::IsMP3(const std::string &filename) {
  return 0 == mp3dec_detect(filename.c_str());
}

bool MP3::IsMP3(const std::vector<uint8_t> &bytes) {
  return 0 == mp3dec_detect_buf(bytes.data(), bytes.size());
}

// Create the MP3 representation after a successful load; frees the
// underlying buffer.
static std::unique_ptr<MP3> FillFromInfo([[maybe_unused]] mp3dec_t *mp3d,
  ...
  mp3->layer = info->layer;
  mp3->bitrate_kbps = info->avg_bitrate_kbps;
  mp3->samples.resize(info->samples);
  memmove((void*)mp3->samples.data(), (void*)info->buffer,
...
std::unique_ptr<MP3> MP3::Load(const std::string &filename) {
  mp3dec_t mp3d;
  mp3dec_init(&mp3d);
  ...
  return FillFromInfo(&mp3d, &info);
}

std::unique_ptr<MP3> MP3::Decode(const std::vector<uint8_t> &bytes) {
  mp3dec_t mp3d;
  mp3dec_init(&mp3d);
