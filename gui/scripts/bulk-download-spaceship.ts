...
async function go() {
  fs.mkdirSync(path.join(__dirname, `../../puzzles/spaceship`), { recursive: true });
  for (let i = 1; i <= 25; i++) {
    try {
      const spaceshipPuzzle = await simpleStringReq(`get spaceship${i}`);
      const fpath = path.join(__dirname, `../../puzzles/spaceship/spaceship${i}.txt`);
      ...
      if (e instanceof DecodeError) {
        const fpath = path.join(__dirname, `../../puzzles/spaceship/spaceship${i}.icfp`);
